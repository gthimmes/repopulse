...
// RenderHTML returns the full self-contained HTML report.
func RenderHTML(data types.MoodResult, meta types.RepoMeta, delta *types.MoodDelta) string {
...
<body>
  <div class="container">
    ...
    <div class="mood-badge">
      ...
      <div class="mood-meta">%s &middot; %dD &middot; %d COMMITS</div>
    </div>
    ...
    <!-- Footer -->
    <div class="footer">
      Generated by <strong>mood-ring</strong> on %s
      &middot; %s – %s
    </div>
  </div>
  ...
  <script>
    ...
    function sortTable(colIndex) {
      ...
      rows.forEach(function(row) {
		...
		data.CompositeScore,
		strings.ToUpper(escapeHTML(meta.RepoName)), meta.WindowDays, meta.AnalyzedCommits,
		narrativeHTML,
		...
		breakdownJS, freqJS, timelineJS, bugJS, coverageInit,
	)
	return sb.String()
}
...
func renderBugExplainability(bug types.BugSignal) string {
    ...
    <summary>
      <span class="chevron">&#x25B6;</span>
      <span class="why-title">Why this score?</span>
      <span class="why-sub">&middot; bug signal breakdown &middot; score %d/100</span>
    </summary>
    <div>%s%s%s</div>
...
func renderTierBlock(tier string, samples []types.BugClassifiedCommit, totalInTier int) string {
	...
	if len(samples) < totalInTier {
		showing = fmt.Sprintf(" (showing %d newest)", len(samples))
	}
	header := fmt.Sprintf(`<div class="why-tier-header %s">
    <span>%s</span>
    <span class="count">· %d commit%s%s</span>
